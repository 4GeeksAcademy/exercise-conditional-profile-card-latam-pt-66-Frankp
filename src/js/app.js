...
function render(variables = {}) {
  // --- TRUCO PARA LIMPIAR LOS SELECTORES ---
  // Esto busca todos los selectores que dicen "null" y los limpia visualmente
  document.querySelectorAll(".picker").forEach(elm => {
    if (elm.value === "null" || elm.value === "") {
      // Opcional: puedes dejarlo vacío "" o poner "Seleccionar..."
      // elm.options[elm.selectedIndex].text = "Seleccionar...";
    }
  });
  // -----------------------------------------
  ...
  // Usamos const como pediste
  const name = variables.name || "Nombre";
  const lastName = variables.lastName || "Apellido";
  const role = variables.role || "Profesión";
  const city = variables.city || "Ciudad";
  const country = variables.country || "País";
